...
- Adjust the truss rod with an allen key (can buy a set of these)
	- Loosen it to allow the neck to go outwards (anti-clockwise when looking down the guitar towards the bridge)
	- Tighten it to allow the neck to go inwards (clockwise when looking down the guitar towards the bridge)
	- Small adjustments are key - check after each one (1/8 to 1/4 turn)
	- Do it until you reach your desired action - 1.75mm on the low E and 1.3mm on the high E is recommended by Milehouse Studios for a good low action
...
- Replace the factory strings ASAP
...
- Big Bends Nut Sauce lubricates the nut slots and the strings as they go through them
...
- For a Gibson-style guitar with three tuners on either side, getting a properly-cut nut is the most important thing for tuning stability - the slots should be angled to match the direction the strings go towards the tuning pegs

## Pickups

- The closer the pickup is to the string, the louder it will be, and vice versa
- If it's too close, it can pull on the strings and cause tuning issues
- Adjust the pickup height using the screws on either side

## Bridge

- String height
	- Set to reach desired action
- Intonation
	- Determines whether your fretted notes are in tune or not
	- Move the saddle towards the bridge if the note is sharp (lengthens the string)
	- Moved the saddle towards the neck if the note is flat (shortens the string)
